...
@app.post("/admin/delete-codelco")
def delete_codelco():
    try:
        with db.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM opportunities WHERE source = 'Codelco'")
                deleted = cur.rowcount
            conn.commit()
        return {"deleted": deleted}
    except Exception as e:
...
@app.post("/admin/delete-mop")
def delete_mop():
    try:
        with db.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM opportunities WHERE source = 'MOP'")
                deleted = cur.rowcount
            conn.commit()
        return {"deleted": deleted}
    except Exception as e:
...
@app.post("/admin/delete-enami")
def delete_enami():
    try:
        with db.get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM opportunities WHERE source = 'ENAMI'")
                deleted = cur.rowcount
            conn.commit()
        return {"deleted": deleted}
    except Exception as e:
